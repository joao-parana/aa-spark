...
object RunRDF {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().master("local["+ cores + "]").appName("spark session example").getOrCreate()
    // import org.apache.spark.sql.SparkSession
    // SparkSession.builder.master("local").appName("spark session example").getOrCreate
    import spark.implicits._

    val dataWithoutHeader = spark.read.
      option("inferSchema", true).
      option("header", true).
      csv("covtype.csv") // "hdfs:///user/ds/covtype.data"
...
class RunRDF(private val spark: SparkSession) {
  ...
  //    */
  //  final def $[T](param: Param[T]): T = getOrDefault(param)
  //
